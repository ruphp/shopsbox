...
Проект находится на стадии формирования ТЗ и архитектуры. Код приложения еще не создан: Symfony skeleton, Docker Compose контур, миграции и структура `src/` пока не заведены.

Git-репозиторий в текущей рабочей папке пока не инициализирован. Это нормальная стартовая точка после переноса: история коммитов не ожидается, а рабочая история проекта сейчас хранится в документации.

Сейчас цель - перейти от подготовки ТЗ к `01 MVP Foundation`. Локальный git-репозиторий и task-ветка `task/01-mvp-foundation` заведены, локальная карточка находится в статусе `In progress` в `docs/workflow/02-current-work-status.md`. Следующий технический шаг - создание Symfony-проекта и Docker Compose контура через `make`-команды.
