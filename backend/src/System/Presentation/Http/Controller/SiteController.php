    ...
    private function staticPages(): array
    {
        ...
        return [
            'privacy' => [
                ...
                'paragraphs' => [
                    $operator,
                    'На сайте shopsbox.ru осуществляется сбор и обработка персональных данных пользователей. При регистрации и использовании сервисов сайта пользователь предоставляет ФИО, адрес электронной почты и номер телефона.',
                    'Эти данные используются для создания и управления интернет-магазином, связи с пользователем, предоставления технической поддержки и исполнения пользовательского соглашения.',
                    'Мы не передаем персональные данные третьим лицам, за исключением случаев, предусмотренных законодательством РФ, а также технических подрядчиков, необходимых для работы сервиса.',
                    'Пользователь может запросить удаление или изменение своих данных, обратившись в службу поддержки по email ruphp@mail.ru.',
                    'Для аналитики посещаемости используется Яндекс.Метрика. При этом собираются обезличенные данные о поведении пользователей на сайте.',
                ],
            ],
            'terms' => [
                'slug' => 'terms',
                ...
                'paragraphs' => [
                    $operator,
                    'Используя shopsbox.ru, пользователь соглашается с настоящими условиями. Платформа предоставляет инструменты для создания интернет-магазинов на поддоменах и дальнейшего подключения собственного домена.',
                    'Пользователь несет полную ответственность за размещенный контент, включая товары, описания, изображения, цены, юридические данные магазина и сведения о доставке.',
            ...
            'payments' => [
                ...
                'paragraphs' => [
                    ...
                    'Оплата производится через защищенные платежные системы. Подключение российских платежных модулей и эквайринга выполняется по мере готовности production-режима.',
            ...
            'contacts' => [
                ...
                'paragraphs' => [
                    'Разработчик платформы: Смирнов Александр Викторович.',
