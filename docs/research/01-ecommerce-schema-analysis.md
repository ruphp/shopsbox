...
- В самом раннем MVP интерфейс может показывать одну розничную цену у варианта, но целевая модель должна вести к отдельному регистру `prices`, чтобы не конфликтовать с 1С, типами цен, каналами продаж и периодами действия.
...
- Для 1С-совместимости все импортируемые сущности должны связываться с внешними GUID через integration mapping. Подробное решение: [1C-compatible catalog model](02-1c-catalog-compatibility.md).
