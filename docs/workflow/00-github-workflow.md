...
## Проверки в конце issue

Перед переводом issue в Done нужно явно ответить, как задача покрыта проверками:

- unit tests - для доменных правил и чистой бизнес-логики;
- integration tests - для связок с базой, репозиториями, миграциями и инфраструктурными адаптерами;
- functional tests - для HTTP/API/страниц и сценариев доступа;
- Docker Compose проверки - для инфраструктуры и локального runtime-контура;
- если новые тесты не нужны, указать причину в PR или финальном отчете.
